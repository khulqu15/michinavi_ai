...
def create_tf_example(group, dataset_path):
    classification = group.object.iloc[0]['classification']
    dataset_path = os.path.join(dataset_path, classification)
...
def main():
    ...
    with tf.io.TFRecordWriter(train_output_path) as writer:
        for group in grouped_train:
            tf_example = create_tf_example(group, dataset_root_path)
