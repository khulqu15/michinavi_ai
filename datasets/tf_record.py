...
def get_class_folder_name(class_id):
    if class_id == 1:
        return 'Berlubang'
    elif class_id == 2:
        return 'Retak Buaya'
    elif class_id == 3:
        return 'Retak Melintang'
    elif class_id == 4:
...
def create_tf_example(group, dataset_path):
    class_name_dict = {1: 'Berlubang', 2: 'Retak Buaya', 3: 'Retak Melintang', 4: 'Retak Memanjang'}

    folder_name = get_class_folder_name(group.object.iloc[0]['class'])
    full_path = os.path.join(dataset_path, folder_name, group.filename)
    ...
    for index, row in group.object.iterrows():
        ...
        class_name = class_name_dict[row['class']]  # Get the class name from the dictionary
        classes_text.append(class_name.encode('utf8'))
        classes.append(row['class'])  # Assume this is the correct class ID
